...
function WordPress_MagicTouch_activate () {
    ...
    $fileContents = file_get_contents(dirname(__FILE__) . '/core/magictouch.css');
    ...
    if($fixedFileContents != $fileContents) {
        file_put_contents(dirname(__FILE__) . '/core/magictouch.css', $fixedFileContents);
    }
    magictoolbox_WordPress_MagicTouch_init() ;

    WordPress_MagicTouch_send_stat('install');

}

function WordPress_MagicTouch_deactivate () {

    //delete_option("WordPressMagicTouchCoreSettings");
    WordPress_MagicTouch_send_stat('uninstall');
}

function WordPress_MagicTouch_send_stat($action = '') {

    //NOTE: don't send from working copy
    if('working' == 'v5.11.12' || 'working' == 'v4.2.1') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magictouch&license=trial&tool_version=v4.2.1&module_version=v5.11.12&platform_name=wordpress&platform_version={$platformVersion}&url={$url}";
...
function showMessage_WordPress_MagicTouch($message, $errormsg = false) {
    if ($errormsg) {
...
function showAdminMessages_WordPress_MagicTouch(){
    global $error_message;
    if (current_user_can('manage_options')) {
       showMessage_WordPress_MagicTouch($error_message,true);
    }
}


function plugin_get_version_WordPress_MagicTouch() {
    $plugin_data = get_plugin_data(str_replace('/plugin.php','.php',__FILE__));
    $plugin_version = $plugin_data['Version'];
    return $plugin_version;
}

function update_plugin_message_WordPress_MagicTouch() {
    ...
    $oldVer = plugin_get_version_WordPress_MagicTouch();
    if (version_compare($oldVer, $ver, '<')) {
        echo '<div id="message" class="updated fade">
                  <p>New version available! We recommend that you download the latest version of the plugin <a href="http://magictoolbox.com/magictouch/modules/wordpress/">here</a>. </p>
              </div>';
    }
}

function MagicTouch_remove_update_nag($value) {
...
function  magictoolbox_WordPress_MagicTouch_init() {

    global $error_message;

    /* add filters and actions into WordPress */
    add_action("admin_menu", "magictoolbox_WordPress_MagicTouch_config_page_menu");

    //add_action("template_redirect", "magictoolbox_WordPress_MagicTouch_styles"); //load scripts and styles only for frontend
	add_action("wp_head", "magictoolbox_WordPress_MagicTouch_styles"); //load scripts and styles

    add_filter("the_content", "magictoolbox_WordPress_MagicTouch_create", 13); //filter content


    add_filter('site_transient_update_plugins', 'MagicTouch_remove_update_nag');
    add_filter( 'plugin_action_links', 'magictoolbox_WordPress_MagicTouch_links', 10, 2 );

    if ($error_message) add_action('admin_notices', 'showAdminMessages_WordPress_MagicTouch');

    //add_filter("shopp_catalog", "magictoolbox_create", 1); //filter content for SHOPP plugin

    if(!isset($GLOBALS['magictoolbox']['WordPressMagicTouch'])) {
        require_once(dirname(__FILE__) . '/core/magictouch.module.core.class.php');
        $coreClassName = "MagicTouchModuleCoreClass";
        $GLOBALS['magictoolbox']['WordPressMagicTouch'] = new $coreClassName;
        $coreClass = &$GLOBALS['magictoolbox']['WordPressMagicTouch'];
    }
    $coreClass = &$GLOBALS['magictoolbox']['WordPressMagicTouch'];
    /* get current settings */
    $settings = get_option("WordPressMagicTouchCoreSettings");
    if($settings !== false && is_array($settings) && !isset($_GET['reset_settings'])) {
        $coreClass->params->appendArray($settings);
    } else {
        update_option("WordPressMagicTouchCoreSettings", $coreClass->params->getArray());
    }
}


function WordPressMagicTouch_config_page() {
     magictoolbox_WordPress_MagicTouch_config_page('WordPressMagicTouch');
}

function magictoolbox_WordPress_MagicTouch_links( $links, $file ) {
    if ( $file == plugin_basename( dirname(__FILE__).'.php' ) ) {
        $links[] = '<a href="plugins.php?page=WordPressMagicTouch-config-page">'.__('Settings').'</a>';
    }
    return $links;
}

function magictoolbox_WordPress_MagicTouch_config_page_menu() {
    if(function_exists("add_submenu_page")) {
        $page = add_submenu_page("plugins.php", __("Magic Touch Plugin Configuration"), __("Magic Touch Configuration"), "manage_options", "WordPressMagicTouch-config-page", "WordPressMagicTouch_config_page");
    }
}

function  magictoolbox_WordPress_MagicTouch_config_page($id) {
    update_plugin_message_WordPress_MagicTouch();
    $settings = $GLOBALS['magictoolbox'][$id]->params->getArray();
    if(isset($_POST["submit"])) {
        /* save settings */
        foreach($settings as $name => $s) {
            if(isset($_POST["magictouchsettings".ucwords(strtolower($name))])) {
                $v = $_POST["magictouchsettings".ucwords(strtolower($name))];
                switch($s["type"]) {
    ...
    $toolAbr = '';
    $abr = explode(" ", strtolower("Magic Touch"));
    ...
    <h2>Magic Touch Settings</h2><br/>
    <p>Learn about all the <a href="http://www.magictoolbox.com/magictouch/integration/" target="_blank">Magic Touch&trade; settings and examples too!</a></p>
    <form action="" method="post" id="magictouch-config-form">
            <?php
                $groups = array();
                ...
                foreach($settings as $name => $s) { 
                    ...
                    $html  .= '<tr>';
                    $html  .= '<th width="50%">';
                    $html  .= '<label for="magictouchsettings'. ucwords(strtolower($name)).'">'.$before.'</label>';
                    ...
                    switch($s["type"]) {
                        case "array": 
                                ...
                                foreach($s["values"] as $p) {
                                    $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="magictouchsettings'.ucwords(strtolower($name)).'" id="magictouchsettings'. ucwords(strtolower($name)).$p.'">';
                                    $pName = ucwords($p);
                                    if(strtolower($p) == "yes")
                        ...
                        default:
                            ...
                            $html .= '<input '.$width.' type="text" name="magictouchsettings'.ucwords(strtolower($name)).'" id="magictouchsettings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                            break;
                    }
            ...
            <p><input type="submit" name="submit" class="button-primary" value="Save settings" />&nbsp;<a href="plugins.php?page=WordPressMagicTouch-config-page&reset_settings=true">Reset to defaults</a></p>
        </form>

   
    </div>
    <div style="font-size:12px;margin:5px auto;text-align:center;">Learn more about the <a href="http://www.magictoolbox.com/magictouch_integration/" target="_blank">customisation options</a></div>
    <?php
}



function  magictoolbox_WordPress_MagicTouch_styles() {
    if(!defined('MAGICTOOLBOX_MAGICTOUCH_HEADERS_LOADED')) {
        $plugin = $GLOBALS['magictoolbox']['WordPressMagicTouch'];
        ...
        echo $headers;
        define('MAGICTOOLBOX_MAGICTOUCH_HEADERS_LOADED', true);
    }
}



function  magictoolbox_WordPress_MagicTouch_create($content) {
    $plugin = $GLOBALS['magictoolbox']['WordPressMagicTouch'];
    /*$pattern = "<img([^>]*)(?:>)(?:[^<]*<\/img>)?";
    $pattern = "(?:<a([^>]*)>.*?){$pattern}(.*?)(?:<\/a>)";*/
    $pattern = "(?:<a([^>]*)>)[^<]*<img([^>]*)(?:>)(?:[^<]*<\/img>)?(.*?)[^<]*?<\/a>";
    ...
    $oldContent = $content;
        $content = preg_replace_callback("/{$pattern}/is","magictoolbox_WordPress_MagicTouch_callback",$content);
        if ($content == $oldContent) return $content;

      

    /*$content = str_replace('{MAGICTOOLBOX_'.strtoupper('magictouch').'_MAIN_IMAGE_SELECTOR}',$GLOBALS['MAGICTOOLBOX_'.strtoupper('magictouch').'_MAIN_IMAGE_SELECTOR'],$content);  //add main image selector to other
    $content = str_replace('{MAGICTOOLBOX_'.strtoupper('magictouch').'_SELECTORS}','',$content); //if no selectors - remove constant
     onlyForModend  */


    if (!$plugin->params->checkValue('template','original') && $plugin->type == 'standard' && isset($GLOBALS['magictoolbox']['MagicTouch']['main'])) {
        ...
        if (!WordPress_MagicTouch_page_check('WordPress')) { //do not render thumbs on category pages
            $thumbs = WordPress_MagicTouch_get_prepared_selectors();
        } else {
            $thumbs = array();
        }
        $html = MagicToolboxTemplateHelperClass::render(array(
            'main' => $GLOBALS['magictoolbox']['MagicTouch']['main'],
        ...
        $html = $GLOBALS['magictoolbox']['MagicTouch']['main'];
        $content = str_replace('MAGICTOOLBOX_PLACEHOLDER', $html, $content);
    }
...
function  magictoolbox_WordPress_MagicTouch_callback($matches) {
    $plugin = $GLOBALS['magictoolbox']['WordPressMagicTouch'];
	...
	$result = preg_replace('/id=\"MagicTouch[^\"]*?'. $id.'\"/is', 'id="'.$id.'"', $result);
...
function WordPress_MagicTouch_get_post_attachments()  {
    $args = array(
